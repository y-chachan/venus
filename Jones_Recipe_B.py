...
path1 = 'C:/Users/YaChan/Desktop/Output_B'   #output folder
path = 'Y:/IoA Summer Project/STEREO/B'
fold_names = [x[0] for x in os.walk(path)]
length = len(fold_names)
l1 = 9
for l1 in range(9,10):    
    path2 = fold_names[i]   #input data
    output_file = os.path.relpath(path2,start='Y:/IoA Summer Project/STEREO/B')
    os.chdir(path2)
    ...
    ilng = 35  #to change the HE longitude, also DIFFERENT for A (-60) and B (35)
    flng = 60  #to change the HE longitude, also DIFFERENT for A (-35) and B (60)
    ilat = -8.5
    flat = 8.5
